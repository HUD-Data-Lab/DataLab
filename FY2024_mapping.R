...
}
{
  ...
  new_CEActivity <- new_CEActivity_AB %>%
    select(-ActivityID) %>%
...
{
  new_HealthAndDV <- HealthAndDV %>%
    select(all_of(healthanddv_columns))
}
{
  new_IncomeBenefits <- IncomeBenefits %>%
...
{
  new_YouthEducationStatus <- YouthEducationStatus %>%
